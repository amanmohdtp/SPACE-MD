...
const games = {}; // chat->sequence

async function handler(msg, args) {
  const chat = msg.key ? msg.key.remoteJid : (msg.remoteJid||'private');
  const arg = (args||'').trim();
  if (!games[chat]) {
    const seq = Array.from({length:3}, ()=>Math.floor(Math.random()*9)+1);
    games[chat]=seq;
    return safeReply(msg,'Memorize sequence: ' + seq.join(' ') + '\nNow repeat with .memory <seq>');
  }
  const given = args.split(' ').map(x=>parseInt(x)).filter(Boolean);
  const ok = JSON.stringify(given)===JSON.stringify(games[chat]);
  delete games[chat];
  return safeReply(msg, ok ? 'Correct!' : 'Wrong. The sequence was ' + games[chat]);
}

if (!autoRegister('.memory', 'Memory game', handler)) {
  module.exports = (client) => registerWithClient(client, '.memory', 'Memory game', handler);
}
