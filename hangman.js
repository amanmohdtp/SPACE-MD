...
const words = ['javascript','hangman','whatsapp','bot','space','node'];
const games = {}; // chat -> {word, display, tries}
...
async function handler(msg, args) {
  ...
  if (!games[chat]) {
    ...
    games[chat] = { word: w, display: Array.from(w).map(_=>'_'), tries: 6 };
    return safeReply(msg, 'Hangman started. Guess letters with .hangman <letter>\n' + games[chat].display.join(' '));
  }
  if (arg.length!==1) return safeReply(msg,'Guess one letter at a time.');
  const g = games[chat];
  let found = false;
  for (let i=0;i<g.word.length;i++) if (g.word[i]===arg) { g.display[i]=arg; found=true; }
  if (!found) g.tries--;
  if (g.display.join('')===g.word) { delete games[chat]; return safeReply(msg,'You win! The word was '+g.word); }
  if (g.tries<=0) { delete games[chat]; return safeReply(msg,'You lost. The word was '+g.word); }
  await safeReply(msg, `Progress: ${g.display.join(' ')}\nTries left: ${g.tries}`);
}

if (!autoRegister('.hangman', 'Play Hangman', handler)) {
  module.exports = (client) => registerWithClient(client, '.hangman', 'Play Hangman', handler);
}
