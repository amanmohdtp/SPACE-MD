...
async function handler(msg, args) {
  ...
  if (!games[chat]) {
    ...
    games[chat] = idx;
    return safeReply(msg, 'Question: ' + questions[idx].q + '\nAnswer with .trivia <your answer>');
  }
  const idx = games[chat];
  if (args.toLowerCase() === questions[idx].a.toLowerCase()) { delete games[chat]; return safeReply(msg,'Correct!'); }
  return safeReply(msg,'Wrong. Try again.');
}

if (!autoRegister('.trivia', 'Trivia quiz', handler)) {
  module.exports = (client) => registerWithClient(client, '.trivia', 'Trivia quiz', handler);
}
