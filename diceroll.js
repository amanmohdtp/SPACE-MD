...
async function handler(msg, args) {
  const sides = parseInt((args||'').split(' ')[0]) || 6;
  const n = Math.floor(Math.random()*sides)+1;
  await safeReply(msg, `🎲 You rolled: ${n} (1-${sides})`);
}

if (!autoRegister('.diceroll', 'Roll dice', handler)) {
  module.exports = (client) => registerWithClient(client, '.diceroll', 'Roll dice', handler);
}
