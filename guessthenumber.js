...
async function handler(msg, args) {
  ...
  if (!games[chat]) {
    ...
    games[chat] = n;
    return safeReply(msg,'I have picked a number between 1 and 100. Guess with .guessthenumber <num>');
  }
  const guess = parseInt(arg);
  if (isNaN(guess)) return safeReply(msg,'Usage: .guessthenumber <number>');
  const target = games[chat];
  if (guess === target) { delete games[chat]; return safeReply(msg, 'Correct! You guessed '+target); }
  if (guess < target) return safeReply(msg, 'Higher!'); else return safeReply(msg, 'Lower!');
}

if (!autoRegister('.guessthenumber', 'Guess the Number', handler)) {
  module.exports = (client) => registerWithClient(client, '.guessthenumber', 'Guess the Number', handler);
}
