...
const games = {}; // chat -> secret (4 digits)
...
async function handler(msg, args) {
  ...
  if (!games[chat]) {
    ...
    games[chat]=secret;
    return safeReply(msg,'Mastermind started. Guess 4-digit code with .mastermind 1234');
  }
  if (!/^[0-9]{4}$/.test(arg)) return safeReply(msg,'Provide a 4-digit guess.');
  const secret = games[chat];
  let bulls=0,cows=0;
  for (let i=0;i<4;i++){ if (arg[i]===secret[i]) bulls++; else if (secret.includes(arg[i])) cows++; }
  if (bulls===4){ delete games[chat]; return safeReply(msg,'You cracked it! '+secret); }
  return safeReply(msg, `Bulls: ${bulls}, Cows: ${cows}`);
}

if (!autoRegister('.mastermind', 'Mastermind', handler)) {
  module.exports = (client) => registerWithClient(client, '.mastermind', 'Mastermind', handler);
}
