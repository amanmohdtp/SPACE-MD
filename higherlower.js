...
const games = {}; // chat -> last number

async function handler(msg, args) {
  const chat = msg.key ? msg.key.remoteJid : (msg.remoteJid||'private');
  const arg = (args||'').trim();
  if (!games[chat]) {
    games[chat] = Math.floor(Math.random()*100)+1;
    return safeReply(msg, 'Starting Higher or Lower. I have a number. Guess higher or lower with .higherlower <higher|lower|number> or .higherlower reset');
  }
  if (arg==='reset') { delete games[chat]; return safeReply(msg,'Reset.'); }
  const current = games[chat];
  if (arg==='higher' || arg==='lower') {
    const next = Math.floor(Math.random()*100)+1;
    const correct = (arg==='higher' && next>current) || (arg==='lower' && next<current);
    games[chat]=next;
    return safeReply(msg, `Next: ${next}. You were ${correct ? 'correct' : 'wrong'}.`);
  }
  const n = parseInt(arg);
  if (!isNaN(n)) {
    const target = games[chat];
    if (n===target) { delete games[chat]; return safeReply(msg,'Correct!'); }
    return safeReply(msg, n < target ? 'Higher' : 'Lower');
  }
  return safeReply(msg,'Usage: .higherlower <higher|lower|number>');
}

if (!autoRegister('.higherlower', 'Higher or Lower', handler)) {
  module.exports = (client) => registerWithClient(client, '.higherlower', 'Higher or Lower', handler);
}
