...
async function handler(msg, args) {
  return safeReply(msg,'Ludo is complex — simplified version not yet implemented. Use multiplayer plugin.');
}

if (!autoRegister('.ludo', 'Ludo (multiplayer)', handler)) {
  module.exports = (client) => registerWithClient(client, '.ludo', 'Ludo (multiplayer)', handler);
}
