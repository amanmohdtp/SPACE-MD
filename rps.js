...
async function handler(msg, args) {
  ...
  if (!user) return safeReply(msg, 'Usage: .rps <rock|paper|scissors>');
  const u = user.toLowerCase();
  if (!choices.includes(u)) return safeReply(msg, 'Invalid choice. Use rock, paper, or scissors.');
  const bot = choices[Math.floor(Math.random()*3)];
  let result = 'Draw!';
  if ((u==='rock'&&bot==='scissors')||(u==='paper'&&bot==='rock')||(u==='scissors'&&bot==='paper')) result='You win!';
  else if (u===bot) result='Draw!'; else result='Bot wins!';
  await safeReply(msg, `You: ${u}\nBot: ${bot}\n\n${result}`);
}

if (!autoRegister('.rps', 'Rock Paper Scissors', handler)) {
  module.exports = (client) => registerWithClient(client, '.rps', 'Rock Paper Scissors', handler);
}
